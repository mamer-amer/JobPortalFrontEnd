...
export class GoogleMapForMeetingComponent implements OnInit {
  ...
  getMeetingRoom() {
    ...
    this.service.getMeetingRoom(this.meetingId, this.userId).subscribe((res) => {
      console.log(res)
      this.meetingObj=res;
      this.meetingLongitude=res.longitude;
      
      this.meetingLatitude=res.latitude;
  ...
  watchLocation() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.watchPosition(function (position) {
        ...
        that.changePosition(position.coords.longitude,position.coords.latitude);
      }, err => { console.log(err) }, that.options);
    }
  }
  //   closeUserSocket() {
  //     if(this.stompClient)
  //     {
  //     this.stompClient.unsubscribe(this.meetingId);
  ...
  gotoUserLocation(longitude,latitude) {

    this.map.setCenter({ lat: latitude, lng: longitude });
    let zoomValue = 15;
    this.zoom = (this.zoom != zoomValue + 0.5) ? zoomValue + 0.5 : zoomValue - 0.5;
